...
Simply replace `mystore.com` with the domain name of your store and then share the link. The great thing about this link is that it's branded and it pre-fills the influencer sign up form fields with your store's influencer commission rules and optional customer discount rules. The sign up page's behaviour can be customised via `customise > sign up page`.
...
You can disabled or customise the behaviour of the widget via the `customise` page
...
Note: You can hide your store from the marketplace via `customise > marketplace`. 
