...
const chunkString = (str: string, length: number) => str.match(new RegExp(`(.|[\r\n]){1,` + length + `}`, `g`))
 ...
 */
export const chunkTransformer = (chunksTotal: any[], node: any) => {

    const htmlChunks = chunkString(node.html, 5000)

    if (htmlChunks != null) {

        const recordChunks = htmlChunks.reduce((recordChunksTotal: any[], htmlChunksItem: string, idx: number) => [
            ...recordChunksTotal,
 ...
 */
const reduceFragmentsUnderHeadings = (accumulator: any[], fragment: any) => {
 ...
 */
export const fragmentTransformer = (recordAccumulator: any[], node: any) => {
    ...
    const records = htmlFragments.reduce((fragmentAccumulator: any[], fragment: any, index: number) => {
